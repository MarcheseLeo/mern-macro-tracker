  ...
  return (
    <header className="d-flex flex-column gap-4 mb-2">
      ...
      <div className="d-flex align-items-center justify-content-between">
        <div>
          <p className="mb-0 dashboard-greetings text-secondary">
            {getGreetings()} ✌️
          </p>
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="position-relative">
            ...
            >
              <Bell size={18} />
              {unreadCount > 0 && areNotificationsEnabled && (
                <span
            ...
            {showNotifs && (
              <div className="notification-panel bg-white shadow-soft rounded-4 p-3">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h6 className="font-heading fw-bold mb-0 text-dark">
                    Notifications
                  </h6>
                ...
                {areNotificationsEnabled ? (
                  <>
                    {notifications.length === 0 ? (
                      <p className="small text-muted mb-0 text-center py-3">
                        No notifications yet.
                      </p>
                    ) : (
                      <div
                        className={`notification-list-wrap ${notifications.length > 4 ? "has-more" : ""}`}
                      >
                        <div className="notification-list d-flex flex-column gap-2 overflow-auto">
                          {notifications.map((n) => (
                            ...
                            >
                              <p
                                className={`small mb-0 ${n.isRead ? "text-muted" : "text-accent-foreground fw-bold"}`}
                              >
                                {n.message}
                              </p>
                            </div>
          ...
          {/* AVATAR */}
          <div
            className="profile-mini-avatar border-0 p-0 cursor-pointer d-flex align-items-center justify-content-center"
      ...
      <div className="d-flex gap-2 overflow-x-auto pb-md-3 mx-2 px-2 no-scrollbar">
        {weekDays.map((day) => {
          ...
          return (
            ...
            >
              <span className="fw-medium">{day.label}</span>
              <span
                className={`font-heading fw-bold lh-1 ${isActive ? "active" : ""}`}
              >
                {day.number}
              </span>
              <span
