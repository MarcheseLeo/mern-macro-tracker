...
export const Calendar = () => {
  const { selectedDate, setSelectedDate, refreshTrigger } =
  ...
  const targetClass =
    selectedDate <= today
      ? `${summary.kcal >= kcalGoal * 0.85 ? "text-success" : `${summary.kcal >= kcalGoal * 0.4 ? "text-warning" : "text-danger"}`}`
      : "text-secondary";
  const targetStr =
    selectedDate <= today
      ? `${summary.kcal >= kcalGoal * 0.85 ? "On target" : `${summary.kcal >= kcalGoal * 0.4 ? "Partial" : "Missed"}`}`
      : "Not available";
      ...
      <section className="app-card p-4">
        <div className="d-flex align-items-center justify-content-between mb-4">
          <button
            ...
            className="btn btn-light rounded-3 d-flex justify-content-center align-items-center p-2 border-0"
          >
            <ChevronLeft size={20} />
          </button>
          ...
          {cells.map((key, i) => {
            ...
            if (key <= today) {
              if (dailyKcal !== undefined && dailyKcal > 0) {
                if (dailyKcal >= kcalGoal * 0.85)
                  dotColor = active ? "bg-light" : "bg-success";
                else if (dailyKcal >= kcalGoal * 0.4)
                  dotColor = active ? "bg-light" : "bg-warning";
            ...
            return (
              ...
              >
                ...
                {dotColor && (
                  <span
                    className={`rounded-circle ${dotColor} mt-1`}
                    style={{ width: "6px", height: "6px" }}
                  ></span>
                )}
                {isToday && !dotColor && !active && (
                  <span
      ...
      >
        <section className="app-card p-4">
          <div className="d-flex justify-content-between align-items-center mb-4 text-dark">
            <h2 className="font-heading fs-5 fw-bold mb-0">
              {summaryDateStr} Summary
            </h2>
            <span className={`small fw-bold ${targetClass}`}>{targetStr}</span>
          </div>
          ...
          <div className="row g-2 mb-4 text-center">
            <div className="col-4">
              <div className="bg-light rounded-3 p-2">
                ...
                <p className="font-heading fw-bold mb-0 text-dark">
                  {Math.round(summary.proteins)}g
                </p>
              </div>
            </div>
            <div className="col-4">
              <div className="bg-light rounded-3 p-2">
                ...
                <p className="font-heading fw-bold mb-0 text-dark">
                  {Math.round(summary.carbs.total)}g
                </p>
              </div>
            </div>
            <div className="col-4">
              <div className="bg-light rounded-3 p-2">
                ...
                <p className="font-heading fw-bold mb-0 text-dark">
                  {Math.round(summary.fats.total)}g
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="d-flex flex-wrap gap-2">
            {["breakfast", "lunch", "dinner", "snack"].map((type) => {
              ...
              return (
                <span
                  key={type}
                  className="badge bg-success-subtle text-success d-flex align-items-center gap-1 py-2 px-3 rounded-pill text-capitalize border border-success-subtle"
                >
                  <Check size={14} /> {type}
                </span>
              );
